...
export const signupCustomer = async (phoneNumber, roleId) => {
  try {
    const response = await axios.post(
      `${process.env.NEXT_PUBLIC_LOGIN_URL}/users/signup`,
      {
        phoneNumber,
        roleId,
      },
...
export const updateCustomer = async (userData, customerToken) => {
  try {
    const response = await axios.post(
      `${process.env.NEXT_PUBLIC_LOGIN_URL}/users/updateUser`,
      userData,
      {
        headers: {
...
export const fetchRcInfo = async (registration, customerToken) => {
  try {
    const response = await axios.post(
      `${process.env.NEXT_PUBLIC_LOGIN_URL}/vehicle/fetchRcInfo`,
      {
        registration,
      },
      {
...
export const assignVehicleToCustomer = async (vehicleId, customerToken) => {
  try {
    const response = await axios.post(
      `${process.env.NEXT_PUBLIC_LOGIN_URL}/vehicle/assignVehicleToCustomer`,
      {
        vehicleId,
      },
      {
